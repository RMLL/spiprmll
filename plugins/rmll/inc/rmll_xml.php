<?php
    ...
    function format_text($text) {
        //$text = supprime_intertitre($texte);
        $text = supprime_img($text);
        $text = propre($text);
        //$text = strip_tags($text);
        //$text = preg_replace("/\r/", "\n", $text);
        //$text = preg_replace("/[\n]+/", "\n", $text);
    ...
    */
    ...
    function do_room($room) {
        $rooms = array(
            'S' => 'Uni_Room_Basement.png',
            'R' => 'Uni_Room_Ground_Floor.png',
            '1' => 'Uni_Room_1st_Floor.png',
            '2' => 'Uni_Room_2nd_Floor.png',
        );
        $room_img = false;
        if (preg_match('/(?P<code>.)\d{3}$/', $room, $matches)) {
            if (array_key_exists($matches['code'], $rooms)) {
                $room_img = sprintf('/rooms/%s', $rooms[$matches['code']]);
            }
        }
        if ($room_img !== false) {
            header(sprintf('Location: %s', $room_img));
            exit(0);
        }
        else {
    ...
    $room = array_key_exists('room', $_GET) ? $_GET['room'] : false;
    if ($room !== false) {
        do_room($room);
    }
    else {
